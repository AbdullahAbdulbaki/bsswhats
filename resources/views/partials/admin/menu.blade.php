        ...
        <ul class="dash-navbar">
            ...
            @if (Auth::user()->type == 'super admin')
                ...
                <li
                    ...
                    <ul
                        class="dash-submenu {{ Request::segment(1) == 'blog_categories' || Request::segment(1) == 'blog_backend' ? ' show' : '' }}">

                            <li class="dash-item {{ Request::route()->getName() == 'blog_categories' ? ' active' : '' }}">
                                <a class="dash-link" href="{{ route('blog_categories.index') }}">{{ __('Blog Categories') }}</a>
                            </li>

                            <li
                                class="dash-item {{ Request::segment(1) == 'blog_backend.index' || Request::route()->getName() == 'blog_backend.create' || Request::route()->getName() == 'blog_backend.edit' ? ' active dash-trigger' : 'collapsed' }}">
                                <a class="dash-link" href="{{ route('blog_backend.index') }}">{{ __('Blog') }}</a>
                            </li>
                ...
                @endcan
                                           {{-- Start Manage Affiliate --}}
        <li
          class="dash-item dash-hasmenu {{ Request::segment(1) == 'configurations' || Request::route()->getName() == 'store.grid' ? ' active dash-trigger' : 'collapsed' }} dash-trigger">
          <a href="#!" class="dash-link">
            <span class="dash-micon">
              <i class="ti ti-home"></i>
            </span>
            <span class="dash-mtext">{{__('Affiliate')}}</span>
          ...
          <ul class="dash-submenu  show" style="box-sizing: border-box; display: block;">
              ...
              @can('Manage affiliate_configurations')
              <li
                class="dash-item {{ Request::segment(1) == 'configurations' || Request::route()->getName() == 'store.grid' ? ' active' : 'collapsed' }}">
                <a href="{{ route('affiliate.configurations') }}"
                  class="dash-link {{ request()->is('affiliate') ? 'active' : '' }}">
                  {{ __('Configurations') }}
                </a>
              </li>
            @endcan
            @can('Manage affiliate_withdraw')
              <li
                class="dash-item {{ Request::segment(2) == 'withdraw-requests' || Request::route()->getName() == 'store.grid' ? ' active' : 'collapsed' }}">
                <a href="{{ route('affiliate.withdraw.index') }}"
                  class="dash-link {{ request()->is('withdraw-requests') ? 'active' : '' }}">
                  {{ __('Withdraw Requests') }}
                </a>
              </li>
            @endcan
            @can('Manage affiliate_earning_histories')
              <li
                class="dash-item {{ Request::segment(2) == 'earning-histories' || Request::route()->getName() == 'store.grid' ? ' active' : 'collapsed' }}">
                <a href="{{ route('affiliate.earnings.index') }}"
                  class="dash-link {{ request()->is('earning-histories') ? 'active' : '' }}">
                  {{ __('Earnings') }}
                </a>
              </li>
            @endcan
            @can('Manage affiliate_payment_histories')
              <li
                class="dash-item dash-hasmenu {{ Request::segment(2) == 'payments' || Request::route()->getName() == 'store.grid' ? ' active dash-trigger' : 'collapsed' }}">
                <a href="{{ route('affiliate.payments.index') }}"
                  class="dash-link {{ request()->is('payments') ? 'active' : '' }}">
            ...
            @else
                ...
                <li
          class="dash-item dash-hasmenu {{ Request::segment(2) == 'affiliate' || Request::segment(2) == 'storeanalytic' || Request::route()->getName() == 'orders.show' ? ' active dash-trigger' : 'collapsed' }}">
          <a href="#!" class="dash-link ">
            <span class="dash-micon">
              <i class="ti ti-home"></i>
            </span>
            <span class="dash-mtext">{{ __('Affiliate System') }}</span>
            <span class="dash-arrow">
              <i data-feather="chevron-right"></i>
            </span>
          </a>
          <ul
            class="dash-submenu {{ Request::segment(2) == 'configure-payouts' || Request::segment(2) == 'storeanalytic' ? ' show' : '' }}">
            @can('Manage affiliate_overview')
              <li class="dash-item {{ Request::route()->getName() == 'affiliate.overview' ? ' active' : '' }}">
                <a class="dash-link" href="{{ route('affiliate.overview') }}">{{ __('overview') }}</a>
              </li>
            @endcan
            @can('Manage payout_configurations')
              <li class="dash-item {{ Request::route()->getName() == 'affiliate.payout.configure' ? ' active' : '' }}">
                <a class="dash-link"
                  href="{{ route('affiliate.payout.configure') }}">{{ __('Payout Configurations') }}</a>
              </li>
            @endcan
            @can('Manage affiliate_earnings')
              <li class="dash-item {{ Request::route()->getName() == 'affiliate.earnings.index' ? ' active' : '' }}">
                <a class="dash-link" href="{{ route('affiliate.earnings.index') }}">{{ __('Earnings Histories') }}</a>
              </li>
            @endcan
            @can('Manage withdraw_requests')
              <li class="dash-item {{ Request::route()->getName() == 'affiliate.withdraw.index' ? ' active' : '' }}">
                <a class="dash-link" href="{{ route('affiliate.withdraw.index') }}">{{ __('Withdraw Requests') }}</a>
              </li>
            @endcan
            @can('Manage payment_histories')
              <li class="dash-item {{ Request::route()->getName() == 'affiliate.payments.index' ? ' active' : '' }}">
                <a class="dash-link" href="{{ route('affiliate.payments.index') }}">{{ __('Payment Histories') }}</a>
              </li>
            @endcan
          </ul>
        </li>
